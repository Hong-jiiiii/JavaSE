 ...
 * 	자바 애블리케이션을 실행하기 위한 가상 머신.
 * 	JVM은 운영체제와 자바애플리케이션 사이 중계자 역할을 한다.
 * 
 * JVM메몰 구조
 * 1. 메서드 영역(Method Area) or 클래스 영역(Class Area)
 * 		JVM이 실행하는 애플리케이션이 사용되는 클래스정보 저장.
 * 		JVM이 시작될 때 클래스 로더에 의해 로드되며, 모든 스레드가 공유.
 * 
 * 2. 힙 영역(Heap Area)
 * 		객체 인스턴스가 저장되는 영역, 동적으로 생성된 객체들이 할당.
 * 
 * 3. 스택 영역(Stack Area)
 ...
 * 	JAVA프로그램이 동적으로 할당한 메모리 중 더이상 사용하지 않는 객체를 자동으로 탐지하고 제거하는 JVM 구성요소.
 * 
 * 
 * 
 */
